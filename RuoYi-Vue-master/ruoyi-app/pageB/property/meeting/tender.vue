  ...
  <view class="vote-container">
    ...
    <view class="meeting-info">
      <view class="meeting-header">
        ...
        <view class="meeting-tag">招标会议</view>
    ...
    <view class="vote-notice">
      ...
      <view class="notice-content">
        <text>1. 请对参与招标的企业进行投票（同意、反对、弃权）。</text>
        <text>2. 选择完所有企业后，点击底部"统一提交投票"按钮。</text>
    ...
    <view class="vote-topics">
      <view 
        ...
        :key="topic.topicId"
      >
        <view class="topic-header company-header">
          <view class="company-logo-wrapper" v-if="topic.avatar">
            <image :src="topic.avatar" class="company-logo" mode="aspectFit"></image>
          </view>
          <view class="header-info">
             <view class="title-row">
               <text class="topic-number">企业{{ index + 1 }}</text>
    ...
    async handleSubmitAll() {
      ...
      if (Object.keys(this.voteData).length < this.topicList.length) {
        const confirm = await uni.showModal({
          title: '提示',
          content: `还有 ${this.topicList.length - Object.keys(this.voteData).length} 个企业未投票，确认提交？`,
          confirmText: '继续提交',
          cancelText: '返回'
        });
...
.meeting-info {
  ...
  .meeting-header {
    ...
    .meeting-tag { background: #FFF7E6; color: #FA8C16; font-size: 22rpx; padding: 4rpx 12rpx; border-radius: 8rpx; margin-right: 10rpx; }
...
.topic-item {
  ...
  .company-header {
    ...
    .company-logo-wrapper { width: 80rpx; height: 80rpx; border-radius: 12rpx; overflow: hidden; margin-right: 20rpx; background: #f5f5f5; .company-logo { width: 100%; height: 100%; } }
    .header-info { flex: 1; }
    .title-row { display: flex; align-items: center; }
    .topic-number { background: #FFF7E6; color: #FA8C16; font-size: 22rpx; padding: 6rpx 12rpx; border-radius: 12rpx; margin-right: 16rpx; }
    .topic-title { font-size: 30rpx; font-weight: 600; color: #262626; }
  }
